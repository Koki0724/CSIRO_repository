...
def preprocess_train_data(train_df, n_splits=5):
    ...
    # y=State (層化), groups=Sampling (グループ)
    # StratifiedGroupKFold は y を層化の基準、groups をグループの基準とします
    for fold, (_, val_idx) in enumerate(sgkf.split(pivot_df, pivot_df['State'], groups=pivot_df['Sampling'])):
